...
TMC2130Stepper::TMC2130Stepper(uint16_t pinEN, GPIO_TypeDef* GPIO_EN, uint16_t pinDIR, GPIO_TypeDef* GPIO_DIR,
...
static SPI_HandleTypeDef* spi = &hspi3;
...
void TMC2130Stepper::begin() {
	...
	if (_pinEN != 0xFFFF) {
		...
		HAL_GPIO_WritePin(GPIO_EN, _pinEN, GPIO_PIN_SET);
	...
	if (_pinSTEP != 0xFFFF) {
		...
		HAL_GPIO_WritePin(GPIO_STEP, _pinSTEP, GPIO_PIN_RESET);
	}

	//pinMode(_pinCS, OUTPUT);
	//digitalWrite(_pinCS, HIGH);
	HAL_GPIO_WritePin(GPIO_CS, _pinCS, GPIO_PIN_SET);
...
//		HAL_GPIO_WritePin(GPIO_CS, _pinCS, GPIO_PIN_RESET);
//
...
//		digitalWrite(_pinCS, HIGH);
		...
		datain[0] = addressByte & 0xFF;
		HAL_SPI_TransmitReceive(spi, datain, dataout, 1, 10);
		status_response = dataout[0];
		...
		if (addressByte >> 7) { // Check if WRITE command
			...
			datain[0] = (*config >> 24) & 0xFF;
			datain[1] = (*config >> 16) & 0xFF;
			datain[2] = (*config >> 8) & 0xFF;
			datain[3] = *config  & 0xFF;

			HAL_SPI_Transmit(spi, datain, 4, 10);
		} else { // READ command
...
//			*config|= SPI.transfer(0x00);
			datain[0] = 0x00;
			HAL_SPI_TransmitReceive(spi, datain, dataout, 4, 10);
			*config = dataout[0];
			*config <<= 8;
			*config |= dataout[1];
			*config <<= 8;
...
bool TMC2130Stepper::isEnabled() { return !HAL_GPIO_ReadPin(GPIO_EN, _pinEN) && toff(); }
